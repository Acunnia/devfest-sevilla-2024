...
import styles from './session-section.module.css';
...
export default function SessionSection({ session }: Props) {
  return (
    <>
      ...
      <div key={session.title} className={styles.container}>
        ...
        <div className={styles['speaker-details']}>
          <div>
            <h1 className={styles.name}>{session.title}</h1>
            <p className={styles.title}>
              <span className={styles.date}>
                {`${new Date(session.start).toLocaleDateString('es-ES', {
                  ...
                  month: 'long',
                  day: 'numeric'
                }).charAt(0).toUpperCase() + new Date(session.start).toLocaleDateString('es-ES', {
                  weekday: 'long',
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
                ...
                <br />
                {`${new Date(session.start).toLocaleTimeString('es-ES', { timeStyle: 'short' })} - ${new Date(session.end).toLocaleTimeString('es-ES', { timeStyle: 'short' })}`}
              </span>
            </p>
            <h2 className={styles['bio-header']}>Descripción</h2>
            <p className={styles.bio}>{session.description}</p>
            <h3 className={styles['socials-header']}>Speaker</h3>
            <div className={styles['speaker-container']}>
              <div className={styles['avatar-container']}>
                <Image
                  loading='lazy'
                  alt={session.speaker[0].image}
                  className={styles.avatar}
                  src={session.speaker[0].image}
                  width={64}
                  height={64}
                  title={session.speaker[0].name}
                />
              </div>
              <p className={styles.speaker_name}>
                {session.speaker[0].name}
              </p>
            </div>
          </div>
        </div>
